        ...
        <div className="container mx-auto px-4 lg:px-12 max-w-[1400px] relative z-20 flex flex-col items-center justify-center text-center pt-20">
           <div className="w-full text-white">
              <h1 className="text-[45px] md:text-[75px] font-black leading-[0.9] mb-6 uppercase tracking-tighter text-white drop-shadow-2xl">
                {isAr ? "نظرة عامة" : "CORPORATE"} <br /> {isAr ? "عامة" : "OVERVIEW"}
              </h1>
              ...
              <div className="flex items-center justify-center gap-3 text-[14px] md:text-[16px] font-black text-white/95 uppercase tracking-[0.2em]">
                ...
                <span className="text-white">{isAr ? "نظرة عامة" : "Overview"}</span>
              </div>
           </div>
        </div>
      </section>
              ...
              <div className="bg-white p-8 md:p-16 shadow-xl border border-gray-100 relative overflow-hidden group">
                 ...
                 <div className="flex flex-col gap-8">
                   <div className="flex flex-col border-l-8 border-alfawad-primary pl-8">
                     <span className="text-alfawad-primary font-black text-[14px] uppercase tracking-[0.4em] mb-2">{isAr ? "تراثنا" : "Our Heritage"}</span>
                     <h2 className="text-[32px] md:text-[45px] font-black text-black leading-tight uppercase tracking-tighter">
                       {isAr ? "الهندسة والبناء" : "Engineering & Construction"}
                     </h2>
                   </div>

                   <p className="text-[20px] md:text-[22px] font-bold text-black leading-relaxed italic border-b border-gray-100 pb-8">
                     {isAr ? "يمثل الفواد منظمة خدمات صناعية متعددة التخصصات تعمل على نطاق واسع في جميع أنحاء المملكة العربية السعودية." : "Al Fawad represents a multi-disciplinary industrial services organization operating extensively throughout the Kingdom of Saudi Arabia."}
                   </p>

                   <div className="text-[#666666] leading-[36px] text-[18px] font-medium flex flex-col gap-6">
                     <p>
                       {isAr ? "مرحباً بكم في عالم الفواد للهندسة والمقاولات حيث تطمحون ونحن نبني! نحن مؤسسة بناء رائدة حاصلة على شهادات الإدارة الجودة والبيئة." : "Welcome to the world of AL FAWAD Engineering & Constructions where you aspire and we build! We are a leading construction establishment with ISO 9001:2015 Quality Management and ISO 14001:2015 Environmental Management certifications."}
                     </p>
                     <p>
                       {isAr ? "من خلال تضافر القوى العاملة الفنية عالية المهارة مع أسطول المعدات الثقيلة الحديث، فإننا نقود إنتاجية لا مثيل لها لعملائنا في القطاعات." : "By synergizing highly-skilled technical manpower with a state-of-the-art heavy equipment fleet, we drive unparalleled productivity for our major clients across the construction, oil & gas, and petrochemical sectors."}
                     </p>
                   </div>

                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
                     {[
                       { title: isAr ? "الخبرة التقنية" : "Technical Expertise", icon: <Settings className="w-8 h-8" />, desc: isAr ? "حلول صناعية شاملة." : "Turn-key industrial solutions spanning across major technical execution divisions." },
                       { title: isAr ? "قوى عاملة ماهرة" : "Skilled Workforce", icon: <Users className="w-8 h-8" />, desc: isAr ? "قاعدة بيانات قوية من الأفراد المهرة المستعدين للتعبئة." : "A robust database of professional, skilled, and semi-skilled personnel mobilization-ready." },
                       { title: isAr ? "أسطول حديث" : "Modern Fleet", icon: <Shield className="w-8 h-8" />, desc: isAr ? "آلات بناء ثقيلة حديثة مدعومة بصيانة وقائية." : "Late-model heavy construction machinery backed by aggresive preventive maintenance." },
                       { title: isAr ? "تميز الجودة" : "QHSE Excellence", icon: <Award className="w-8 h-8" />, desc: isAr ? "مقاييس صارمة للسلامة والجودة تضمن التنفيذ الخالي من الحوادث." : "Uncompromising safety and quality metrics ensuring absolute incident-free execution." }
                     ].map((item, i) => (
                   ...
                   <div className="mt-12 bg-black p-12 flex flex-col md:flex-row items-center gap-10">
                      ...
                      <div className="text-white flex-1">
                         <h4 className="text-2xl font-black uppercase mb-4 tracking-tighter">{isAr ? "التأثير الاستراتيجي" : "Strategic Impact"}</h4>
                         <p className="text-white/60 leading-relaxed font-bold">
                            {isAr ? "انطلاقاً من رؤية 2030، نهدف إلى أن نصبح شريك الصناعة الثقيلة المهيمن." : "Driven by Vision 2030, we are targeting to become the predominant heavy industry partner supporting robust national growth across the Kingdom."}
                         </p>
                      </div>
                   </div>
                 </div>
