        ...
        <div className="container mx-auto px-4 lg:px-12 max-w-[1400px] relative z-20 flex flex-col items-center justify-center text-center pt-20">
          <div className="w-full text-white">
            <h1 className="text-[45px] md:text-[75px] font-black leading-[0.9] mb-6 uppercase tracking-tighter text-white drop-shadow-2xl">
              {isAr ? "تأجير" : "EQUIPMENT"} <br /> {isAr ? "المعدات" : "RENTAL"}
            </h1>
            ...
            <div className="flex items-center justify-center gap-3 text-[14px] md:text-[16px] font-black text-white/95 uppercase tracking-[0.2em]">
              ...
              <ChevronRight className="w-4 h-4 text-white/50" />
              <span className="text-white">{isAr ? "تأجير المعدات" : "Equipment Rental"}</span>
            </div>
          </div>
        </div>
      </section>
              ...
              <div className="bg-white p-8 md:p-16 shadow-xl border border-gray-100 relative overflow-hidden group">
                ...
                <div className="flex flex-col gap-8">
                  <div className="flex flex-col border-l-8 border-alfawad-primary pl-8">
                    <span className="text-alfawad-primary font-black text-[14px] uppercase tracking-[0.4em] mb-2">{isAr ? "حلول الأسطول" : "Fleet Solutions"}</span>
                    <h2 className="text-[32px] md:text-[45px] font-black text-black leading-tight uppercase tracking-tighter">
                      {isAr ? "خدمات تأجير المعدات" : "Equipment Rental Services"}
                    </h2>
                  </div>

                  <p className="text-[20px] md:text-[22px] font-bold text-black leading-relaxed italic border-b border-gray-100 pb-8">
                    {isAr ? "نحن نقدم أسطولًا شاملاً من المعدات الثقيلة والخفيفة لتلبية الاحتياجات المتغيرة لقطاعات البناء والصيانة." : "We offer a comprehensive fleet of heavy and light equipment to meet the dynamic needs of construction, industrial maintenance, and logistics sectors."}
                  </p>
                  ...
                  <div className="bg-black p-12 mt-4 relative overflow-hidden group">
                    ...
                    <h3 className="text-2xl font-black text-white uppercase mb-8 border-l-4 border-alfawad-primary pl-6 tracking-tighter">{isAr ? "يشمل أسطول معداتنا" : "Our Equipment Fleet Include"}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-4">
                    {[
                      { title: isAr ? "خدمات المشغل" : "Operator Services", icon: <Truck className="w-8 h-8" />, desc: isAr ? "مشغلين معتمدين وذوي خبرة." : "Certified and experienced operators for all equipment categories." },
                      { title: isAr ? "دعم في الموقع" : "On-Site Support", icon: <Zap className="w-8 h-8" />, desc: isAr ? "تعبئة سريعة ودعم فني لضمان عدم توقف العمل." : "Fast mobilization and technical support to ensure zero downtime." },
                      { title: isAr ? "متوافق مع السلامة" : "Safety Compliant", icon: <ShieldCheck className="w-8 h-8" />, desc: isAr ? "جميع الآلات تحمل شهادات تفتيش ضرورية." : "All machines carry necessary inspection and valid third-party certificates." }
                    ].map((item, i) => (
