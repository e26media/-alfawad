...
const ServiceSidebar = () => {
  ...
  const SERVICE_LINKS = [
     { label: isAr ? "خدمة المواد" : "MATERIAL SERVICE", path: "/industrial/services/material-service" },
    { label: isAr ? "القوى العاملة الفنية" : "TECHNICAL MANPOWER", path: "/industrial/services/technical-manpower" },
    { label: isAr ? "المعدات الثقيلة" : "HEAVY EQUIPMENT", path: "/industrial/services/heavy-equipment" },
    ...
    <div className="w-full lg:w-1/3 xl:w-1/4 shrink-0">
      ...
      <div className="bg-white shadow-xl border border-gray-100 overflow-hidden mb-8">
        <h3 className="bg-[#1c1c1c] text-white font-black uppercase text-[18px] px-8 py-5 flex items-center justify-between tracking-tighter">
          {isAr ? "خدماتنا" : "Our Services"} <div className="w-2.5 h-2.5 rounded-full bg-alfawad-primary" />
        </h3>
      ...
      <div className="bg-alfawad-primary p-8 text-white text-center flex flex-col items-center gap-6 shadow-2xl relative overflow-hidden group">
        ...
        <h4 className="text-xl font-black uppercase tracking-tighter relative z-10 leading-none">{isAr ? "بحاجة للدعم؟" : "Need Support?"} <br/> {isAr ? "احصل على تسعيرة" : "Get a Quote"}</h4>
        <p className="text-white/80 text-sm font-bold relative z-10 leading-relaxed uppercase tracking-wider">{isAr ? "تحدث إلى خبرائنا" : "Talk to our experts"}</p>
        <Link to="/industrial/contact" className="bg-black text-white px-8 py-4 font-black uppercase text-xs tracking-widest w-full transition-all hover:bg-white hover:text-black relative z-10 shadow-lg hover:-translate-y-1 block text-center">
